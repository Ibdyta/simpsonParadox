...
# Install gridExtra for side-by-side plots (more compatible than patchwork)
...
``` r
# Create the side-by-side comparison using gridExtra
comparison_plot <- grid.arrange(
  plot1, plot2,
  ncol = 2,
  top = "Simpson's Paradox: The Same Data, Two Different Stories",
  bottom = "Left: Overall relationship appears positive | Right: Within-species relationships are actually negative"
)
```
...
This comparison makes Simpson’s Paradox impossible to miss: - **Left
plot:** Shows the misleading positive relationship when we ignore
species - **Right plot:** Reveals the true negative relationships within
each species - **The contrast is stark:** The same data tells completely
different stories depending on our analytical approach
